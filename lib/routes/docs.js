        ...
        options: {
            ...
            async handler(request, h) {
                ...
                const getDocs = async (file) => {
                    ...
                    return await request.services().github.markdown(owner, name, file, (ref || !version) ? ref : `v${version}`);
                };

                const htmls = await Promise.all(docsFiles.map(getDocs));
                const versionedPackageName = pkg.npm && request.services().npm.packageName(pkg, (ref || !version) ? 'latest' : version);
