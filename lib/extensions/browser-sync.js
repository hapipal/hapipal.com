...
module.exports = [
    {
        ...
        method: (server, next) => {
            ...
            bs.watch(`${base}/templates/partials/**/*.hbs`).on('change', (file) => {

                const { manager } = server.realm.plugins.vision;
                const engine = manager._engines.hbs;

                manager._loadPartials(engine);
            });
